...
func (q *Query) First(column string) error {
...
func (q *Query) Last(column string) error {
	return q.OrderExpr("? DESC", column).Limit(1).Scan()
}

func (q *Query) Scan(values ...interface{}) (e error) {
	if q.stickyErr != nil {
		return q.stickyErr
	}
	...
	_, e = q.query(q.ctx, q.model, NewSelectQuery(q))
	if e != nil {
		return e
	}
